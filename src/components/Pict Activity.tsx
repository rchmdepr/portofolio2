  ...
  return (
    <section id="activities" className="py-20 bg-background">
      <div className="container mx-auto px-6">
        ...
        <div className="relative max-w-5xl mx-auto group min-h-[300px]">
          {/* Tombol Navigasi Kiri (Floating) - Selalu Muncul */}
          <button 
            onClick={() => scroll('left')}
            className="absolute left-2 top-1/2 -translate-y-1/2 z-20 p-2 md:p-3 rounded-full bg-background/80 border border-slate-800 text-slate-200 hover:text-primary hover:border-primary hover:bg-slate-900/90 backdrop-blur-sm transition-all duration-300 shadow-lg active:scale-95 flex items-center justify-center"
            aria-label="Previous slide"
          >
            <ChevronLeft className="w-5 h-5 md:w-6 md:h-6" />
          </button>

          {/* Tombol Navigasi Kanan (Floating) - Selalu Muncul */}
          <button 
            onClick={() => scroll('right')}
            className="absolute right-2 top-1/2 -translate-y-1/2 z-20 p-2 md:p-3 rounded-full bg-background/80 border border-slate-800 text-slate-200 hover:text-primary hover:border-primary hover:bg-slate-900/90 backdrop-blur-sm transition-all duration-300 shadow-lg active:scale-95 flex items-center justify-center"
          ...
          <div 
            ref={scrollRef}
            className="flex overflow-x-auto gap-6 pb-12 snap-x snap-mandatory w-full px-4 [&::-webkit-scrollbar]:hidden"
